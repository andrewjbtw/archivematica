...
def archivematicaGetRights(metadataAppliesToList, fileUUID):
    ...
    for metadataAppliesToidentifier, metadataAppliesToType in metadataAppliesToList:
        fields = "RightsStatement.pk, rightsStatementIdentifierType, rightsStatementIdentifierValue, rightsBasis"
        key = fields.split(", ")
        sql = """SELECT %s FROM RightsStatement WHERE metadataAppliesToidentifier = '%s' AND metadataAppliesToType = '%s';""" % (fields, metadataAppliesToidentifier, metadataAppliesToType)
        rows = databaseInterface.queryAllSQL(sql)
        for row in rows:
            valueDic = dict(zip(key, row))
            rightsStatement = createRightsStatement(valueDic, fileUUID)
            ret.append(rightsStatement)
    return ret


def createRightsStatement(valueDic, fileUUID):
    # valueDic should contain keys: RightsStatement.pk, rightsStatementIdentifierType, rightsStatementIdentifierValue, rightsBasis
    ...
    if valueDic["rightsBasis"] in RIGHTS_BASIS_OTHER:
    ...
    if valueDic["rightsBasis"].lower() in ["copyright"]:
        ...
        for row2 in rows2:
            ...
            etree.SubElement(copyrightInformation, ns.premisBNS + "copyrightStatus").text = row2[1]
            copyrightJurisdiction = row2[2]
            ...
            etree.SubElement(copyrightInformation, ns.premisBNS + "copyrightStatusDeterminationDate").text = formatDate(row2[3])
            ...
            if row2[4]:  # copyrightApplicableStartDate
                etree.SubElement(copyrightApplicableDates, ns.premisBNS + "startDate").text = formatDate(row2[4])
            if row2[6]:  # copyrightApplicableEndDateOpen
                etree.SubElement(copyrightApplicableDates, ns.premisBNS + "endDate").text = "OPEN"
            elif row2[5]:  # copyrightApplicableEndDate
                etree.SubElement(copyrightApplicableDates, ns.premisBNS + "endDate").text = formatDate(row2[5])
        ...
        for row2 in rows2:
            ...
            etree.SubElement(licenseInformation, ns.premisBNS + "licenseTerms").text = row2[0]
            ...
            if row2[1]:
                etree.SubElement(licenseApplicableDates, ns.premisBNS + "startDate").text = formatDate(row2[1])
            if row2[7]:  # LicenseApplicableEndDateOpen
                etree.SubElement(licenseApplicableDates, ns.premisBNS + "endDate").text = "OPEN"
            elif row2[2]:
                etree.SubElement(licenseApplicableDates, ns.premisBNS + "endDate").text = formatDate(row2[2])
        ...
        for row2 in rows2:
            ...
            if not otherRightsBasis or valueDic["rightsBasis"] in RIGHTS_BASIS_OTHER:  # not 100%
