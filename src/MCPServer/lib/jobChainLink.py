...
class jobChainLink:
    ...
    def getNextChainLinkPK(self, exitCode):
        if exitCode is not None:
            try:
                link = MicroServiceChainLinkExitCode.objects.get(microservicechainlink_id=str(self.pk), exitcode=str(exitCode))
                return link.nextmicroservicechainlink_id
    ...
    def updateExitMessage(self, exitCode):
        ...
        if exitCode is not None:
            try:
                code = MicroServiceChainLinkExitCode.objects.get(microservicechainlink_id=str(self.pk), exitcode=str(exitCode))
                message = code.exitmessage
